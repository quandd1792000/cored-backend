"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TermOfServiceService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const mongodb_1 = require("mongodb");
const term_of_service_entity_1 = require("./entity/term-of-service.entity");
let TermOfServiceService = class TermOfServiceService {
    constructor(termOfServiceRepo) {
        this.termOfServiceRepo = termOfServiceRepo;
    }
    async findAllTermOfService() {
        const termOfServices = await this.termOfServiceRepo.find();
        if (termOfServices) {
            termOfServices.forEach((Element) => {
                Element._id = Element._id.toString();
            });
            return termOfServices;
        }
        else {
            return null;
        }
    }
    async createTermOfService(data) {
        const termOfService = this.termOfServiceRepo.create(data);
        await this.termOfServiceRepo.save(data);
        return termOfService;
    }
    async updateTermOfService(id, updateData) {
        await this.termOfServiceRepo.update(id, updateData);
        return await this.termOfServiceRepo.findOne({
            _id: (0, mongodb_1.ObjectId)(id),
        });
    }
    async findTermOfService(Query) {
        const termOfService = await this.termOfServiceRepo.findOne(Query);
        if (termOfService) {
            termOfService._id = termOfService._id.toString();
            return termOfService;
        }
        else {
            return null;
        }
    }
    async removeTermOfService(id) {
        const termOfService = await this.termOfServiceRepo.delete(id);
        return termOfService;
    }
};
TermOfServiceService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(term_of_service_entity_1.TermOfServiceEntity)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], TermOfServiceService);
exports.TermOfServiceService = TermOfServiceService;
//# sourceMappingURL=term-of-service.service.js.map